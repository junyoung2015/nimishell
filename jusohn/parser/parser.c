...
t_ast_node *parse_cmd(t_token **tokens, t_size *token_idx, t_size num_tokens)
{
    ...
    while (*token_idx < num_tokens && ((*tokens)[*token_idx].type == TOKEN_WORD || (*tokens)[*token_idx].type == TOKEN_ENV_VAR))
    {
...
{
    t_ast_node  *right_cmd_node;
    t_ast_node  *left_cmd_node;

    left_cmd_node = parse_cmd(tokens, token_idx, num_tokens);
    if (!left_cmd_node)
        return (0);
    // Check for pipe token
    while (*token_idx < num_tokens && (*tokens)[*token_idx].type == TOKEN_PIPE)
    {
        // Skip the pipe token
        (*token_idx)++;
        right_cmd_node = parse_cmd(tokens, token_idx, num_tokens);
        if (!right_cmd_node)
            return (0);
        ...
        pipe_node->left = left_cmd_node;
        pipe_node->right = right_cmd_node;
        // The new pipe node becomes the left node for the next iteration
        left_cmd_node = pipe_node;
    }
